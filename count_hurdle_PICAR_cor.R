...
filename="CorPICAR_count_hurdle_run.RData"
CVfilename="CV_CorPICAR_count_hurdle_run.RData"
...
# par(mfrow=c(2,4))
# for(i in 1:2){
#   plot(x=dimSeq,y=rsResults[[1]][i,],typ="l")
#   plot(x=dimSeq,y=rsResults[[2]][[i]][1,],typ="l")
#   plot(x=dimSeq,y=rsResults[[2]][[i]][2,],typ="l")
#   plot(x=dimSeq,y=rsResults[[2]][[i]][3,],typ="l")
# }
...
MBasis_P<-as.matrix(AMat[posInd,]%*%base_P)
MQM_O<-diag(m_O)
MQM_P<-diag(m_P)

invMQM_O<-diag(m_O)
invMQM_P<-diag(m_P)
LA<-t(chol(MBasis_O%*%t(MBasis_O)+diag(n_O)*0.00001))
MBasis_P_full<-as.matrix(AMat%*%base_P)
LB<-t(chol(MBasis_P_full%*%t(MBasis_P_full)+diag(n_O)*0.00001))
LAtLB<-LA%*%t(LB)
matA<-solve(MBasis_P_full%*%t(MBasis_P_full)+diag(n_O)*0.00001, MBasis_P_full)
matB<-LAtLB%*%matA
baseCor<-t(MBasis_O)%*%solve((MBasis_O%*%t(MBasis_O)+diag(n_O)*0.00001),matB)


X_O<-cbind(1,ObsDat$XMat.mod)
X_P<-cbind(1,ObsDat$XMat.mod[posInd,])
p_O<-ncol(X_O)
p_P<-ncol(X_P)
###############################################################
n=length(ObsDat$obs.mod) 
...
# plot(density(samples_extended[thin,parInd[1]]));abline(v=ObsDat$parTruth[3],col='red')
# plot(density(samples_extended[thin,parInd[2]]));abline(v=ObsDat$parTruth[4],col='red')
# plot(density(1/samples_extended[thin,parInd[5]]))
# plot(density(samples_extended[thin,parInd[3]]));abline(v=ObsDat$parTruth[7],col='red')
# plot(density(samples_extended[thin,parInd[4]]));abline(v=ObsDat$parTruth[8],col='red')
# plot(density(1/samples_extended[thin,parInd[6]]))
...
# PLot Random Effects
# burnin<-floor(nrow(samples_extended)*0.7)
# dO_ind<-grep(pattern="delta_O", colnames(samples_extended))
# dP_ind<-grep(pattern="delta_P", colnames(samples_extended))
# re<-apply(t(as.matrix(AMat%*%base_O)%*%t(samples_extended[-(1:burnin),dO_ind])),2,mean)
# par(mfrow=c(2,2))
# plotRF(dat=re,rangeDat=c(ObsDat$gpS.mod,re),label="O-Model",location=ObsDat$grid.location.mod,length.out=100)
# plotRF(dat=ObsDat$gpS.mod,rangeDat=c(ObsDat$gpS.mod,re),label="O-Truth",location=ObsDat$grid.location.mod,length.out=100)
# 
# re<-apply(t(as.matrix(AMat%*%base_P)%*%t(samples_extended[-(1:burnin),dP_ind])),2,mean)[posInd]
# plotRF(dat=re,rangeDat=c(ObsDat$gpW.mod,re),label="P-Model",location=ObsDat$grid.location.mod[posInd,],length.out=100)
# plotRF(dat=ObsDat$gpW.mod[posInd],rangeDat=c(ObsDat$gpW.mod,re[posInd]),label="P-Truth",
#        location=ObsDat$grid.location.mod[posInd,],length.out=100)

# > cvMSPE_ZIP[[1]] #RMSPE
# [1] 2.312732 2.228965
# > cvMSPE_ZIP[[2]] #Precision
# [,1]      [,2]
# [1,] 0.447227 0.5170608
# > cvMSPE_ZIP[[3]] #Recall
# [,1]      [,2]
# [1,] 0.4499422 0.5908399
# > cvMSPE_ZIP[[4]] #F1
# [,1]      [,2]
# [1,] 0.4485805 0.5514938
